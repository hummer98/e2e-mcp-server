...
E2E MCP Serverは、AIエージェント（Claude Code、ChatGPT、Gemini等）がE2Eテストを実行する際のサーバー管理の煩雑さを解消します。従来のPlaywright MCPではAIエージェント自身が開発サーバーの状態管理を行う必要がありましたが、本サーバーはサーバープロセスのライフサイクル管理をMCP層に移譲し、AIエージェントがテストロジックに専念できる環境を提供します。
...
- **E2Eセッションの完全自動化**: MCPツール呼び出しで開発サーバー起動からテスト実行、サーバー停止までを完全自動化
- **PlaywrightのMCPツール化**: Playwrightの主要機能をMCPツールとして提供し、既存Playwright MCPと同等の操作性を実現
- **デバッグ情報の自動収集**: エラー発生時のサーバーログ、スクリーンショット、構造化エラー詳細を自動収集し、AIエージェントの根本原因分析を支援
- **複数AIクライアント対応**: Claude Code、ChatGPT、Geminiから標準的なMCPプロトコルでアクセス可能

### 設計思想

- **サーバーコマンド非依存**: ユーザー提供のサーバー起動コマンド（bash、Node.js、Python等）をJSON応答インターフェースで統合
- **セッション分離**: 1セッション = 1開発サーバー + 1Browserインスタンスで状態汚染を防止
- **AI優先のエラーハンドリング**: AIエージェントの自己修復能力を最大化する構造化エラー、スクリーンショット、ログ提供
- **クラウド対応アーキテクチャ**: Cloud Runやコンテナ環境に適したステートレス設計
