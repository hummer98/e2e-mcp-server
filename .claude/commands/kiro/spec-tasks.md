...
6. **E2E test scenarios**: For user-facing features, include a task to create E2E test scenarios in `docs/e2e/scenarios/`
   - Use template: `docs/e2e/scenario-template.md`
   - Include browser window size standardization (1194x834)
   - Reference common steps from `docs/e2e/common-steps.md`
   - Register scenarios in `docs/e2e/scenarios-local.md` and `docs/e2e/scenarios-production.md`
7. **PoC exclusions**: For proof-of-concept projects, EXCLUDE the following task categories:
   - Performance optimization tasks (React.memo, memoization, bundle size optimization, etc.)
   - Accessibility improvement tasks (ARIA labels, keyboard navigation, screen reader support, WCAG compliance, etc.)
   - Production readiness tasks (monitoring, analytics, observability)
   - Include ONLY if explicitly required in requirements.md
